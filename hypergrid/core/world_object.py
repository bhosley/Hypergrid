...
class WorldObj(np.ndarray, metaclass=WorldObjMeta):
    ...
    def __new__(cls, type: str | None = None, color: str = Color.from_index(0)):
        ...
        obj.init_pos: tuple[int, int] | None = (
            None  # initial position of the object
        )
        obj.cur_pos: Sequence[int] | None = (
            None  # current position of the object
        )
...
###
# TODO: Specific Objects Built below, likely constrained to 2d?
...
class Lava(WorldObj):
    ...
    def render(self, img):
        ...
        for i in range(3):
            ...
            yhi = 0.4 + 0.2 * i
            fill_coords(
                img, point_in_line(0.1, ylo, 0.3, yhi, r=0.03), (0, 0, 0)
            )
            fill_coords(
                img, point_in_line(0.3, yhi, 0.5, ylo, r=0.03), (0, 0, 0)
            )
            fill_coords(
                img, point_in_line(0.5, ylo, 0.7, yhi, r=0.03), (0, 0, 0)
            )
            fill_coords(
                img, point_in_line(0.7, yhi, 0.9, ylo, r=0.03), (0, 0, 0)
            )
...
class Door(WorldObj):
    ...
    def is_open(self, value: bool):
        ...
        if value:
            ...
            self.state = (
                State.closed
            )  # set state to closed (unless already locked)
    ...
    def is_locked(self, value: bool):
        ...
        if value:
            ...
            self.state = (
                State.closed
            )  # set state to closed (unless already open)
...
class Box(WorldObj):
    ...
    def __new__(
        cls, color: str = Color.yellow, contains: WorldObj | None = None
    ):
        """
        Parameters
        ----------
    ...
    def render(self, img):
        ...
        # Outline
        fill_coords(
            img, point_in_rect(0.12, 0.88, 0.12, 0.88), self.color.rgb()
        )
        fill_coords(img, point_in_rect(0.18, 0.82, 0.18, 0.82), (0, 0, 0))

        # Horizontal slit
        fill_coords(
            img, point_in_rect(0.16, 0.84, 0.47, 0.53), self.color.rgb()
        )
