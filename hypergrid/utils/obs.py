...
## ----- Full Information Functions ----- ##


# @nb.njit(cache=True)
# njit does not play nice with isinstance, meshgrid or itertools
def get_view_coords(
    ...
    # Coordinates are the cartesian of the view dims
    # translated by the position and orientation of the agent
    coords = [
        np.array(np.meshgrid(*(np.arange(-half, half + 1),) * N)).T
        + (pos + np.multiply(dir, half))
        for dir, half, pos in zip(agent_dir, half_views, agent_pos)
    ]
    # Rotate the view to compensate for agent orientation
    oriented_coords = []
    for oriented, a_dir in zip(coords, agent_dir):
        ...
        for ax, d_0n in enumerate(a_dir[1:]):
            if d_0n != 0:
                oriented = np.rot90(oriented, k=-d_0n, axes=(0, ax + 1))
...
def gen_obs_grids(
    ...
    for view_coords in agent_view_coords:
        ...
        # )
        obs_grid = np.full(
            (*view_coords.shape[:-1], WorldObj.dim), WorldObj.empty()
        )
        ...
        for obs_idx in np.ndindex(view_coords.shape[:-1]):
            ...
            # Negative index implies out of bounds
            if np.any(np.array(grid_idx) < 0):
                obs_grid[obs_idx] = UNSEEN_ENCODING
            else:
                try:
                    obs_grid[obs_idx] = grid_encoding[*grid_idx]
                except IndexError:
                    # Catch upper out-of-bounds
    ...
    for agent, o_grid in enumerate(obs_grid):
        if not see_through_walls[agent]:
            ...
            for mask_idx in np.ndindex(vis_mask.shape):
                # TODO: Faster if vis_mask doesn't contain walls
                if not (
                    vis_mask[mask_idx]
                    or np.array_equal(obs_grid[agent][*mask_idx], WALL_ENCODING)
                ):
